...
function showProducts(goods, filterBy){
    ...
    goods.forEach(item => {
        ...
        <a href="${item.href}" class="tile__link">
            ...
            ${(item.badge ==="sale")?`<span class="tile__sale-info sale">
...
function getSearchParams(key) {
    ...
    getGoods().then(result => {
        ...
        calculateAndRenderTimeToEndSale()
        setSort(result);
        setFilter(result);
...
const bannerEl = document.querySelector(".js-sale-banner")
const bannerCloseEl = document.querySelector(".js-sale-banner-close")
const timeRepitBanner = 604800000

bannerCloseEl.addEventListener('click', closeBanner)
statusBannerAfterReload()

function statusBannerAfterReload(){
    if (localStorage.getItem("timeCloseBanner")) {
        const timeAfterrCloseBanner = new Date().getTime() - localStorage.getItem("timeCloseBanner")
        if (timeAfterrCloseBanner > timeRepitBanner)
        {
            bannerEl.style.display = "flex"
        }
        else{
            bannerEl.style.display = "none"
            setTimeout(()=>{ bannerEl.style.display = "flex"}, timeAfterrCloseBanner)
        }
    }
    else{
        bannerEl.style.display = "flex"
...
function closeBanner(){
    console.log(bannerEl.style)
    bannerEl.style.display = "none"
    const closeTime = new Date().getTime()
    localStorage.setItem("timeCloseBanner", closeTime)
    setTimeout(()=>{ bannerEl.style.display = "flex"},timeRepitBanner)
}
...
function calculateAndRenderTimeToEndSale(){
    const endSaleTime = new Date('April 1, 2023 00:00:00')
    const arraySaleCounterEl = document.querySelectorAll(".sale__counter")
    
    setInterval(()=>{
        arraySaleCounterEl.forEach((el)=>{
            const currentTime = new Date();
            const difTime = endSaleTime.getTime() -currentTime.getTime()
            const difSecond = Math.floor(difTime/1000);
            const difMinuts = Math.floor(difSecond/60);
            const difHours = Math.floor(difMinuts/60);
            const difDay = Math.floor(difHours/24);  
            el.innerText = `${difDay} дня ${difHours%24} год ${difMinuts%60} хв ${difSecond%60} сек`     
            })
        }, 1000)
}
